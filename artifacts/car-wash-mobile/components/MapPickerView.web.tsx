...
.pin{position:absolute;top:calc(50% - 80px/2);left:50%;transform:translate(-50%,-100%);z-index:999;pointer-events:none;transition:transform .12s ease}
...
.result-item:active,.result-item:hover{background:#f0f4ff}
.result-name{font-size:14px;font-weight:600;color:#111}
.result-sub{font-size:12px;color:#888;margin-top:2px}
.locate-btn{position:absolute;bottom:90px;right:12px;z-index:1000;background:#2563EB;border:none;border-radius:50%;width:44px;height:44px;box-shadow:0 2px 12px rgba(37,99,235,0.4);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:opacity .15s}
.locate-btn:active{opacity:.75}
.locate-btn.locating{opacity:.6;cursor:default}
.footer{position:absolute;bottom:0;left:0;right:0;height:80px;background:#0A1628;padding:8px 16px 12px;z-index:1000;border-top:1px solid rgba(255,255,255,0.09)}
...
// Locate button — ask parent React layer for location (works across iframe sandboxes)
locateBtn.addEventListener('click',function(){
  if(locating)return;
  locating=true;
  locateBtn.classList.add('locating');
  locateBtn.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="10" stroke-dasharray="31.4" stroke-dashoffset="10"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur=".8s" repeatCount="indefinite"/></circle></svg>';
  sendMsg({type:'requestLocation'});
  // Auto-reset spinner after 12 s in case parent doesn't respond
  setTimeout(function(){
    locating=false;
    locateBtn.classList.remove('locating');
    locateBtn.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/><circle cx="12" cy="12" r="8" stroke-opacity=".5"/></svg>';
  },12000);
});
...
const MapPickerView = forwardRef<MapPickerViewRef, MapPickerViewProps>(
  function MapPickerView({ initialLocation, onLocationChange, onRequestLocation }, ref) {
    ...
    useEffect(() => {
      const attach = () => {
        const domEl = containerRef.current as HTMLElement | null;
        if (!domEl) return;

        if (iframeRef.current) {
          try { domEl.removeChild(iframeRef.current); } catch {}
    ...
    useEffect(() => {
      return () => {
        if (iframeRef.current) {
          try { iframeRef.current.parentElement?.removeChild(iframeRef.current); } catch {}
          iframeRef.current = null;
        }
        if (blobUrlRef.current) {
          URL.revokeObjectURL(blobUrlRef.current);
    ...
    return (
      <View style={styles.container}>
        ...
        <View ref={containerRef} style={styles.mapWrap} />
      </View>
    );
  }
);
