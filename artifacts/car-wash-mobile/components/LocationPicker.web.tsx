...
clearBtn.addEventListener('click',function(){
  searchInput.value='';clearBtn.style.display='none';searchResults.style.display='none';searchInput.focus();
});

document.getElementById('locateBtn').addEventListener('click',function(){
  if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(
      function(p){map.setView([p.coords.latitude,p.coords.longitude],17);},function(){}
    );
  }
});
...
document.getElementById('confirmBtn').addEventListener('click',function(){
  var c=map.getCenter();
  sendMsg({type:'confirm',lat:c.lat,lng:c.lng,address:currentAddress||addrDisplay.textContent});
});
